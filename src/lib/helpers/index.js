...
export const baseURL = (import.meta.env.VITE_API_BASE_URL || (import.meta.env.DEV
  ? "http://localhost:8000"
  : "https://about.connectize.co")).replace(/\/$/, "");
...
export async function makeApiRequest({
  ...
  const requestUrl = url.startsWith('/api') && import.meta.env.DEV 
    ? url 
  ...
  try {
    ...
    console.log('Making API request:', { url: requestUrl, method });
    ...
    if (response.status >= 200 && response.status <= 204) {
      ...
      if (method !== 'GET') {
        console.log('API request successful:', { url, status: response.status, hasData: !!response.data });
      }
