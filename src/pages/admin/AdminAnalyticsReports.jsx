...
const AdminAnalyticsReports = () => {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const fetchPage = async (basePath, { page = 1, page_size = 50, extra = '' } = {}) => {  // Reduced from 200
        const qs = `?page=${page}&page_size=${page_size}${extra ? `&${extra.replace(/^&/, '')}` : ''}`;
        return await safe(`${basePath}${basePath.endsWith('/') ? '' : '/'}${qs}`);
      };
      const fetchAllPaged = async (basePath, { page_size = 50, maxPages = 10, extra = '' } = {}) => {  // Reduced from 200 and 25
        const all = [];
