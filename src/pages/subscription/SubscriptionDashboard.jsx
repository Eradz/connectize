...
const SubscriptionDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Additional debug: Log what we're setting for dashboard data
      console.log('🔍 Final Dashboard Data:', {
        'plans.length': (plansResult?.data?.results || []).length,
        'features.length': (allFeatures || []).length,
        'featuresCategories.keys': Object.keys(featuresData),
        'featuresCategories.totalFeatures': Object.values(featuresData).flat().length,
        'analytics': !!analyticsResult?.data,
        'currentSubscription': !!extractedSubscription,
        'planDetails': (plansResult?.data?.results || []).map(p => ({ id: p.id, name: p.name, plan_type: p.plan_type })),
        'featureSource': featuresResult?.data?.source || 'unknown'
      });
  ...
  const getPlanFeatures = (planType) => {
    if (!dashboardData.features.length) {
      console.log(`🔍 getPlanFeatures(${planType}): No features available (dashboardData.features.length = ${dashboardData.features.length})`);
      return [];
    }
    ...
    const filteredFeatures = dashboardData.features.filter(feature => 
      availablePlans.includes(feature.minimum_plan?.toLowerCase())
    );
    
    console.log(`🔍 getPlanFeatures(${planType}): ${filteredFeatures.length} features (total: ${dashboardData.features.length}, plans: ${availablePlans.join(',')})`);
  ...
  const handleViewPlanDetails = (plan) => {
    console.log('🔍 handleViewPlanDetails called with plan:', plan);
    
    if (!plan) {
      console.error('❌ No plan provided to handleViewPlanDetails');
      return;
    }
    
    if (!plan.id) {
      console.error('❌ Plan has no ID:', plan);
      // Fallback: Try to show plan details inline or use plan_type as ID
      if (plan.plan_type) {
        console.log('🔄 Attempting fallback navigation with plan_type:', plan.plan_type);
    ...
    try {
      ...
      // Fallback: stay on current page and maybe show a modal or alert
      alert(`Plan details for ${plan.name} - ID: ${plan.id}`);
    }
  };
