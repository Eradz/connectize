...
const WorkforceApplications = () => {
  ...
  const filteredApplications = useMemo(() => {
    ...
    if (activeTab !== 'all') {
      filtered = filtered.filter(app => {
        switch (activeTab) {
          case 'active':
            return ['pending', 'under_review', 'interview_scheduled', 'interviewed'].includes(app.status);
          case 'completed':
            return ['offered', 'accepted', 'rejected', 'withdrawn'].includes(app.status);
          case 'interviews':
  ...
  const getTabCount = (tab) => {
    switch (tab) {
      ...
      case 'active':
        return applications.filter(app => ['pending', 'under_review', 'interview_scheduled', 'interviewed'].includes(app.status)).length;
      case 'completed':
        return applications.filter(app => ['offered', 'accepted', 'rejected', 'withdrawn'].includes(app.status)).length;
      case 'interviews':
        return applications.filter(app => app.status === 'interview_scheduled' || app.status === 'interviewed').length;
      default:
        return 0;
    }
  };
