...
const WorkforceEventDetail = () => {
  ...
  useEffect(() => {
    ...
    if (event && user) {
      // Check if user is the creator (check both created_by and organizer fields)
      const isCreator = event.created_by === user.id || event.organizer === user.id;
  ...
  const loadParticipants = async () => {
    try {
      ...
      if (isEventCreator) {
        ...
        const response = await workforceAPI.getEventRegistrations(event.id);
        console.log('Participants API response:', response);
        if (response.success && response.data?.results) {
          setParticipants(response.data.results);
          console.log('Loaded participants:', response.data.results.length);
        } else if (response.data?.results) {
          setParticipants(response.data.results);
          console.log('Loaded participants (no success flag):', response.data.results.length);
        } else {
          console.log('No participants data in response:', response);
          setParticipants([]);
        }
      } else {
                ...
                <div className="space-y-1">
                  {event.speakers.map((speaker, index) => (
                    <div key={index} className="flex items-center space-x-2">
                      ...
                      <div className="flex-1">
                        <h3 className="font-medium text-gray-900 text-xs">{speaker.name}</h3>
