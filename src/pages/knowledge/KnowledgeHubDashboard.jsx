...
const KnowledgeHubDashboard = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      setFeaturedArticles(featuredRes?.data || featuredRes || []);
      setTrendingArticles(trendingRes?.data || trendingRes || []);
      setRecentArticles(recentRes?.results || recentRes?.data || []);
      setForums(forumsRes?.results || forumsRes?.data || []);
      setCategories(categoriesRes?.results || categoriesRes?.data || []);
      setPopularTags(tagsRes?.data || tagsRes || []);
      ...
      // Calculate stats
      const totalViews = (recentRes?.results || []).reduce((sum, article) => sum + (article.views || 0), 0);
      const totalTopics = (forumsRes?.results || []).reduce((sum, forum) => sum + (forum.topic_count || 0), 0);
      ...
      setStats({
        ...
        totalForums: (forumsRes?.results || []).length,
        totalTopics,
        totalViews,
        activeUsers: 1200, // This would come from a user analytics API
        onlineUsers: 45    // This would come from a real-time API
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            ...
            <div className="mt-4 flex items-center text-sm">
              <span className="text-gray-600">{stats.onlineUsers || 0} online now</span>
            </div>
          </div>
        </div>
